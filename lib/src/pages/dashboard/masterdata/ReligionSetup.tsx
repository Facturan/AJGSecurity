...
export function ReligionSetup() {
  const { religions, addReligion } = useMasterData();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submitStatus, setSubmitStatus] = useState<'idle' | 'success' | 'error'>('idle');
  const [submitMessage, setSubmitMessage] = useState('');

  const handleAdd = async () => {
    const inputEl = document.getElementById('religionName') as HTMLInputElement;
    ...
    try {
      await addReligion(val);
      setSubmitStatus('success');
      setSubmitMessage('Religion added successfully!');
      inputEl.value = '';
      setTimeout(() => setSubmitStatus('idle'), 4000);
    } catch (err: any) {
      setSubmitStatus('error');
      setSubmitMessage(err.message || 'Failed to add Religion');
    } finally {
