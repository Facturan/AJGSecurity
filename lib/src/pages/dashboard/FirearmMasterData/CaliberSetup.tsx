...
export function CaliberSetup() {
    const { firearmCalibers, addFirearmCaliber, updateFirearmCaliber, deleteFirearmCaliber } = useMasterData();
    ...
    const handleAdd = async () => {
        ...
        await addFirearmCaliber(newName.trim());
        setNewName('');
    };
    ...
    const handleUpdate = async () => {
        ...
        await updateFirearmCaliber(editId, editValue.trim());
        cancelEdit();
    };

    const handleDelete = async (id: number) => {
        if (!confirm('Delete this caliber? This cannot be undone.')) return;
        await deleteFirearmCaliber(id);
    };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in slide-in-from-bottom-2 duration-300">
            ...
            <Card className="border-none shadow-xl shadow-slate-200/50 rounded-3xl overflow-hidden">
                <CardHeader className="bg-slate-50/50 border-b border-slate-100 p-6">
                    <CardTitle className="text-xl font-black text-slate-800 tracking-tight">Add New Caliber</CardTitle>
                    <CardDescription className="text-xs font-medium text-slate-500">Register a new firearm caliber in the master data.</CardDescription>
                </CardHeader>
                <CardContent className="p-8">
                    <div className="space-y-2">
                        <Label className="text-[11px] font-extrabold text-slate-500 uppercase tracking-widest ml-1">Caliber Name</Label>
            ...
            <Card className="border-none shadow-xl shadow-slate-200/50 rounded-3xl overflow-hidden">
                <CardHeader className="bg-slate-50/50 border-b border-slate-100 p-6">
                    <div className="flex items-center justify-between">
                        <div>
                            <CardTitle className="text-xl font-black text-slate-800 tracking-tight">Existing Calibers</CardTitle>
                            <CardDescription className="text-xs font-medium text-slate-500">View, edit, or delete current calibers.</CardDescription>
                        </div>
                        <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-black text-xs">{firearmCalibers.length}</div>
                    </div>
                </CardHeader>
                <CardContent className="p-6">
                    <div className="space-y-2 max-h-[500px] overflow-y-auto pr-2">
                        {firearmCalibers.map(item => (
                        ...
                        {firearmCalibers.length === 0 && <p className="text-center text-slate-400 text-sm py-8">No calibers added yet.</p>}
                    </div>
                </CardContent>
            </Card>
        </div>
