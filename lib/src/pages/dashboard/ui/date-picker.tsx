...
export function DatePicker({ value, onChange, placeholder = 'MM/DD/YYYY', className, disabled }: DatePickerProps) {
    ...
    return (
        <Popover open={open} onOpenChange={setOpen}>
            <div
                className={cn(
                    'group flex min-h-[32px] w-full items-center justify-between rounded-md border border-input bg-white px-2 py-1 text-sm ring-offset-background',
            ...
            >
                <div className="flex items-center gap-2 flex-1 min-w-0">
                    <input
                        ...
                        onBlur={handleBlur}
                        className="flex-1 bg-transparent border-0 outline-none text-slate-800 placeholder:text-slate-400 text-xs font-bold py-0"
                        maxLength={10}
                    />
                    <PopoverTrigger asChild>
                        <button type="button" disabled={disabled} className="focus:outline-none flex items-center shrink-0">
                            <CalendarIcon className="h-3.5 w-3.5 text-slate-400 group-hover:text-blue-500 transition-colors cursor-pointer" />
                        </button>
                    </PopoverTrigger>
                </div>
                {selected && (
                    <span
                        onClick={handleClear}
                        className="ml-1 shrink-0 rounded-full p-0.5 hover:bg-slate-100 text-slate-400 hover:text-slate-600 transition-colors cursor-pointer"
                    >
                        <X className="h-3 w-3" />
                    </span>
                )}
